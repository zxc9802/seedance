...
主站 `SSO_USAGE_SECRETS` JSON 中的 `seedance` 密钥必须与这里相同。不要使用 NEXT_PUBLIC_/VITE_ 前缀，不要提交真实密钥。
...
Seedance 的同步图片、文案按每次模型请求统计；异步视频和聚合图片按数据库任务终态统计，轮询不重复计数。`upstream_cost_cny` 来自现有供应商报价，标记为 CNY 估算金额；不使用 sale_price_cny 或 charged_credits 作为供应商成本。异步任务未提供 Token 时显示未知。
