...
export function createUsageMonitor({ tool, fetchImpl = (...args) => globalThis.fetch(...args), config = defaultConfig, autoDrain = true }) {
  ...
  async function observedFetch(input, init) {
    ...
    const polledId = isPoll ? text(body.taskId ?? body.task_id ?? body.id ?? url.searchParams.get('taskId')
      ?? url.searchParams.get('task_id') ?? url.searchParams.get('id') ?? url.pathname.match(/\/(?:videos\/generations|videos|tasks|query|requests|responses)\/([^/]+)/i)?.[1]) : null;
    if (isPoll) {
      ...
      try {
        if (response.ok) {
          ...
          const relayFailure = /^\/v1\/videos\/generations\/[^/]+\/?$/.test(url.pathname)
            && ['failed', 'failure', 'cancelled', 'canceled', 'interrupted', 'incomplete'].includes(responseState(payload));
          if (status && (returnedTaskId || relayFailure || (!payload?.error && payload?.success !== false))) {
