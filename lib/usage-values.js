...
const count = (value) => typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= 1_000_000_000 ? value : null;
/** Counts include cached input and reasoning output; do not add them to totals twice. */
export function parseTokenUsage(value) {
...
export function mergeUsage(previous, next) {
    ...
    if (next.totalTokens === null && result.inputTokens !== null && result.outputTokens !== null)
        result.totalTokens = result.inputTokens + result.outputTokens;
    return result;
}
