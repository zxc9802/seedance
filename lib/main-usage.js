...
import { emptyUsage, mergeUsage, parseTokenUsage } from './usage-values.js';
const users = new AsyncLocalStorage();
export function runWithUsageUser(userId, action) { return users.run(userId, action); }
...
function filename(id) { return createHash('sha256').update(`${process.env.USAGE_TOOL}:${id}`).digest('hex'); }
let flushing = null;
let retryAt = 0;
let failures = 0;
let timer;
async function save(event, extension) {
    const dir = directory();
...
export async function queueUsageReport(event) {
    if (!isUsageReportingEnabled())
        return;
    if (!event.userId || !event.requestId)
        throw new Error('Usage report requires server-authenticated identity and request id');
...
export async function flushUsageReports() {
    if (!isUsageReportingEnabled() || Date.now() < retryAt)
        return;
    if (flushing)
        return flushing;
    flushing = (async () => {
        ...
        for (const name of names.filter(n => n.endsWith('.pending')).slice(0, 50)) {
            ...
            if (!info || Date.now() - info.mtimeMs < 24 * 3600_000)
                continue;
        ...
        for (const name of names.filter(n => /^[a-f0-9]{64}\.json$/.test(n)).slice(0, 50)) {
            ...
            if (!['https:', 'http:'].includes(base.protocol))
                throw new Error('Invalid main application URL');
            ...
            if (!response.ok || payload?.success !== true)
                throw new Error(`Usage report HTTP ${response.status}`);
            // Do not delete a newer queued version while this request was in flight.
            if (await readFile(file, 'utf8').catch(() => '') === body)
                await unlink(file).catch(() => undefined);
        }
        failures = 0;
        retryAt = 0;
...
async function beginUsage(userId, url, init) {
    let body = {};
    if (typeof init?.body === 'string') {
        try {
            body = JSON.parse(init.body);
        }
        catch { /* Not a JSON request. */ }
    }
...
export async function meteredFetch(input, init, explicitUserId, transport = fetch) {
    const userId = explicitUserId ?? users.getStore();
    if (!isUsageReportingEnabled())
        return transport(input, init);
    if (!userId)
        throw new Error('Usage reporting requires an authenticated employee');
    ...
    if (body.stream === true && url.pathname.endsWith('/chat/completions')) {
        init = { ...init, body: JSON.stringify({ ...body, stream_options: { ...(body.stream_options ?? {}), include_usage: true } }) };
    }
    let response;
    try {
        ...
        await finish({ ...event, status: 'failed' });
        throw error;
    }
    ...
    function observe(value) {
        ...
        if (value && typeof value === 'object') {
            ...
            if (['message_stop', 'response.completed', 'response.failed', 'response.incomplete'].includes(type))
                terminal = true;
            if (['error', 'response.failed', 'response.incomplete'].includes(type) || ('error' in value && Boolean(value.error)))
                failed = true;
        }
        Object.assign(event, mergeUsage(event, parseTokenUsage(value)));
        if (url.pathname.endsWith('/embeddings') && event.inputTokens !== null) {
    ...
    if (!response.body || !response.headers.get('content-type')?.includes('text/event-stream')) {
        ...
        try {
            observe(JSON.parse(new TextDecoder().decode(bytes)));
        }
        catch { /* Preserve missing usage. */ }
        ...
        async pull(controller) {
            try {
                ...
                if (part.done) {
                    ...
                    await complete(failed ? 'failed' : terminal ? 'completed' : 'interrupted');
                    controller.close();
                }
                else {
                    consume(decoder.decode(part.value, { stream: true }));
                    if (terminal)
                        await complete(failed ? 'failed' : 'completed');
...
async function finish(event) {
    ...
    try {
        await queueUsageReport(event);
    }
    catch {
        console.error('[usage-report] Unable to save final usage; pending request retained for reconciliation.');
    }
}
if (isUsageReportingEnabled()) {
    startWorker();
