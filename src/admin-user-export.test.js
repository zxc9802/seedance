...
test('main admin remains the original usage dashboard without credit-center cards', async () => {
  const adminSource = await readFile(new URL('../admin/index.html', import.meta.url), 'utf8')

  assert.match(adminSource, /VEO Studio 用量监控/)
  assert.match(adminSource, /费用导入/)
  assert.match(adminSource, /成功率/)
  assert.doesNotMatch(adminSource, /剩余积分/)
  assert.doesNotMatch(adminSource, /消耗积分/)
})
