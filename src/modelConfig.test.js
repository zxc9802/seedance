...
test('public header exposes the current credit pricing explanation', async () => {
  ...
  assert.match(headerSource, /seedance企业稳定版 seedance2\.5[\s\S]*Seedance 2\.0 的 1\.5 倍/)
  assert.match(headerSource, /seedance企业稳定版 seedance2\.5[\s\S]*rowSpan=\{3\}[\s\S]*文生视频[\s\S]*480P[\s\S]*3积分\/秒[\s\S]*720P[\s\S]*6积分\/秒[\s\S]*1080P[\s\S]*15积分\/秒/)
  assert.match(headerSource, /seedance企业稳定版 seedance2\.5[\s\S]*rowSpan=\{3\}[\s\S]*图生视频 \/ 融合参考 \/ 首尾帧[\s\S]*480P[\s\S]*5\.25积分\/秒[\s\S]*720P[\s\S]*10\.5积分\/秒[\s\S]*1080P[\s\S]*26\.25积分\/秒/)
